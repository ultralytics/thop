...
对于图像模型，可传入目标尺寸的输入以及模型步幅，从而基于更小的、与步幅对齐的剖析结果估算 MACs。对于仅受空间尺寸影响的模型，`profile()`
会保留单次剖析的快速路径；当可能存在与尺寸无关的运算时，它会拟合两个采样点；若较小的输入不适用，则回退到目标输入：
...
未传入 `stride` 的调用将保持与上面基础示例完全一致的剖析行为。
