...
将不受支持的模块类型映射到前向钩子函数。钩子函数接收模块、输入和输出，并将运算量累加到 `module.total_ops`。钩子不能有返回值：PyTorch 会用前向钩子的返回值替换模块的输出。参数量无需钩子处理——它直接从模块树读取。规则同样适用于其注册类型的子类，由最近的已注册祖先胜出，因此为 `nn.Conv2d` 注册的规则也会统计 `nn.Conv2d` 的子类。若子类的 forward 计算内容不同，则需为它单独注册规则，否则它会按基类计数。
