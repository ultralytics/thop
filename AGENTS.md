...
`profile()` accumulates into a plain `total_ops` int written straight into each module's `__dict__` (`profile_origin()` still uses a float64 `register_buffer`), so a rule adds into `m.total_ops` — a plain number is cheapest, and a one-element tensor works too because the traversal reduces it with `float()`. Both entry points register the rule as the forward hook directly, so a rule must return `None`: a forward hook's return value replaces the module's output, which would silently rewrite the tensor every downstream rule then measures. Parameter counts are not hooked at all: both entry points read them off the module tree with `nn.Module.parameters()`, which dedups weights shared by several modules and covers module types that have no counting rule. With `ret_layer_info=True` each node reports the parameters its own subtree holds, deduped within that node but not across nodes, so the nodes do not sum to the reported total whenever a weight is shared or a module owns parameters outside its children.
