...
`profile()` accumulates into a plain `total_ops` int written straight into each module's `__dict__` (`profile_origin()` still uses a float64 `register_buffer`), and registers each counting function as the forward hook directly — so a rule must add plain Python numbers, never tensors, into `m.total_ops` and return `None`, because a forward hook that returns a value replaces the module's output. Parameter counts are not hooked at all: both entry points read them off the module tree with `nn.Module.parameters()`, which dedups weights shared by several modules and covers module types that have no counting rule. With `ret_layer_info=True` each node reports the parameters its own subtree holds, deduped within that node but not across nodes, so under weight tying the nodes do not sum to the reported total.
